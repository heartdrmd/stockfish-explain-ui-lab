...
function installNotationInput(target, navigate, { enabled = () => true, keyboard = true, selector = "[data-move-notation]" } = {}) {
	...
	const wheel = (event) => {
		if (!available(event) || !event.target?.closest?.(selector) || !event.deltaY || Math.abs(event.deltaX) > Math.abs(event.deltaY)) return;
		event.preventDefault();
