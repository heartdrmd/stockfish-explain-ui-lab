...
function validClockProgram(value) {
	const p = value;
	if (!p || !["dgt", "zmf"].includes(p.family) || !validConfig(p.config)) return false;
	const c = p.config, max = p.family === "dgt" ? 4 : 3;
	if (c.periods.length > max || p.slot != null && (!Number.isInteger(p.slot) || (p.family === "dgt" ? p.slot < 26 || p.slot > 30 : p.slot < 1 || p.slot > 3))) return false;
	return c.periods.every((s, i) => {
		if (s.method === "END") return false;
		if (i === 0 && s.method !== "UPCNT" && s.time.some((t) => t <= 0)) return false;
		if ([
			"BYO",
			"CAN-BYO",
			"UPCNT",
			"HOURGLASS"
		].includes(s.method) && i !== c.periods.length - 1) return false;
		if (s.time.some((t) => !Number.isInteger(t) || t > (p.family === "dgt" ? 35999e3 : 35994e4))) return false;
		if (s.extra.some((t) => !Number.isInteger(t) || t > (p.family === "dgt" ? 599e3 : 6e4))) return false;
		if (s.moments.some((n) => !Number.isInteger(n) || n > 99)) return false;
		if (p.family === "dgt") return s.method !== "HOURGLASS" && (!s.moves || s.method === "FISCH") && (s.method !== "CAN-BYO" || s.time.every((t) => t > 0 && t <= 599e3)) && (s.method !== "BYO" || s.time.every((t) => t > 0));
		if ([
			"DELAY",
			"BYO",
			"CAN-BYO"
		].includes(s.method)) return false;
		if (s.method === "HOURGLASS") return i === 0 && s.time.every((t) => t >= 5e3 && t <= 18e4);
		if (s.method === "UPCNT") return i === 1 && c.periods[0].method === "TIME" && c.periods[0].time.every((t) => t >= 6e4 && t <= 24e5);
		return !s.moves || c.tournament;
	});
}
function controlFromProgram(program) {
	const first = program.config.periods[0];
	return {
		minutes: first.time[1] / 6e4,
		incrementSeconds: first.method === "FISCH" ? first.extra[1] / 1e3 : 0,
		program: JSON.parse(JSON.stringify(program))
	};
}
function clockControlLabel(control) {
	const program = control.program;
	if (!program) return `${control.minutes}+${control.incrementSeconds}`;
	const names = {
		TIME: "Sudden death",
		FISCH: "Fischer increment",
		DELAY: "Bronstein delay",
		"US-DLY": "US delay",
		BYO: "Japanese byo-yomi",
		"CAN-BYO": "Canadian byo-yomi",
		UPCNT: "Count up",
		HOURGLASS: "Hourglass"
	};
	return program.config.periods.map((p, i) => `${program.config.periods.length > 1 ? `P${i + 1}: ` : ""}${names[p.method]}${p.method !== "UPCNT" ? ` · ${p.time[1] / 6e4} min` : ""}${[
		"FISCH",
		"DELAY",
		"US-DLY"
	].includes(p.method) ? ` + ${p.extra[1] / 1e3}s` : ""}${p.moves ? ` / ${p.moves} moves` : ""}`).join(" → ");
}
...
function readClockControl(value) {
	const c = value;
	if (c?.program !== void 0) return validClockProgram(c.program) ? controlFromProgram(c.program) : null;
...
function resetClockControl(clock, value, turn, now) {
	...
	if (!control) throw new Error("Check the time, increment, delay and period settings.");
	...
	if (control.program) {
		const engine = new ClockHardware(control.program.family);
		engine.config = JSON.parse(JSON.stringify(control.program.config));
		engine.reset();
		Object.assign(clock, {
			mode: engine.config.periods[0].method === "UPCNT" ? "up" : "down",
			displayOnly: false,
			initialMs,
			msWhite: engine.times[1],
			msBlack: engine.times[0],
	...
	if ("timeControl" in clock) delete clock.timeControl;
	Object.assign(clock, {
		mode: "down",
		displayOnly: false,
		initialMs,
...
//#endregion
export { CLOCK_CONTROL_PRESETS, clockControlLabel, controlFromProgram, readClockControl, resetClockControl, startUntimedDisplay, usesClockBudget, validClockProgram };
