...
test('shared clocks use parent time in both board modes and reject foreign controls', t => {
  ...
  const steps = [];
  board.backward = () => steps.push(-1);
  board.forward = () => steps.push(1);
  message('zagreb:navigate-step', {}, origin, {direction:-1});
  message('zagreb:navigate-step', frame.contentWindow, 'https://other.example', {direction:-1});
  message('zagreb:navigate-step', frame.contentWindow, origin, {direction:'1'});
  message('zagreb:navigate-step', frame.contentWindow, origin, {direction:2});
  assert.deepEqual(steps, [], 'Only valid directions from the board frame can navigate');
  message('zagreb:navigate-step', frame.contentWindow, origin, {direction:-1});
  message('zagreb:navigate-step', frame.contentWindow, origin, {direction:1});
  ...
  message('zagreb:navigate-step', frame.contentWindow, origin, {direction:1});
  board.watchActive = false;
  assert.deepEqual(steps, [-1,1], 'Locked positions and watched games never navigate the underlying study');
