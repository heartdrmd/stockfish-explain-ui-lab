...
test('clock docking moves the same live card above tools and returns it home',t=>{
  ...
  board.setClockDock('right');assert.equal(right.child,card);assert.equal(opened,0,'A right clock does not reopen hidden analysis, including on preference restore');
  ...
  assert.equal(opened,1,'Moving back to a hidden left pane reveals the clock');
