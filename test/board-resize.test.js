...
function fixture(split = true) {
  ...
  installBoardResizeHandle({
    handle, boardElement, workspaceSplit: { isActive: () => split }, blurTarget,
    request: callback => { frames.set(++nextFrame, callback); return nextFrame; },
    cancel: id => frames.delete(id),
    onStart: () => {},
    onResize: width => { applied.push(width); if (split) columnWidth = width; else squareWidth = width; },
    onFinish: moved => finished.push({ moved, width: split ? columnWidth : squareWidth }),
  });
  return {
...
test('a height-limited 2D corner drag starts at the current column width', () => {
  const f = fixture();
  f.pointer('pointerdown');
